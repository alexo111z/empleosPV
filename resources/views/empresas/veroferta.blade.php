...
<main role="main">
    ...
    <div class="row">
        ...
        {!! Form::open(array('route'=>array('empresas.editOferta',$oferta->id),'method'=>'POST', 'id'=>'form-oferta')) !!}
            {{ csrf_field() }}
            <div class="col-sm-12 px-3 pt-3">
                <div class="form-group">
                <label for="titulo">Título</label>
                    <input autocomplete="off"  type="text" class="form-control" id="titulo" name="titulo" placeholder="Ej. Desarrollador web" value="{{ $oferta->titulo}}" required>
                </div>
            </div>
            <div class="col-sm-12 px-3 py-0 my-0 ">
                <div class="form-group">
                <label for="desc_corta">Descripción breve de la oferta</label>
                    <textarea class="form-control mb-3 text-desc" id="desc_corta" name="desc_corta" rows="3" style="display: inline-block;" required>{{ $oferta->d_corta }}</textarea>
                </div>
            </div>
            ...
            <div class="row">
            <div class="row ml-0 pl-0 col-sm-12 px-3 mt-0 pt-0">
                ...
                <div class="col-sm-6 lugar">
                    ...
                    <select id="CmbEstado" name='estado' class="form-control" onchange='funcestado(this.value,<?php echo json_encode($ciudades); ?>)'>
                        <option selected disabled hidden>Seleccionar....</option>
                        @foreach($estados as $estado)
                            @if($estado->id == $oferta->id_estado)
            ...
            <div class="col-sm-12 px-3 mt-1">
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="descripcion" id="descripcion" rows="5">{{ $oferta->d_larga }}</textarea>
                </div>
            </div>
            <hr class="mb-4">

            <div class="col-md-12 mb-0 ml-0  text-right ">
                <button id="BtnCancelar" onclick="" type="button " class=" btn-secondary form-inline icon btn">Cancelar</button>
                <button id="BtnGuardar" data-href="{{ route('admin.edit.ofr', ['oferta'=>$oferta->id]) }}" type="button " class="btn-info form-inline icon btn">Guardar</button>
            </div>
            {!! Form::close() !!}
        </div>
       
        
        <div id="opciones" class="col-md-3 my-3 px-auto mx-auto" style="min-width: 250px;">
            ...
            <div class=" border rounded overflow-hidden text-center mx-auto py-3">
                ...
                <div class="text-center mx-auto" >
                        <div class="col-sm-12 my-3 mx-auto text-center">
                            <button id="BtnEditar" type="submit" class="btn btn-postular btn-block "><h5>Editar mi oferta</h5></button>
                        </div>
                        <div class="col-sm-12 my-3 mx-auto text-center">
                            {{ csrf_field() }}
...
@section('scripts')
   ...
   <script src="{{asset('js/EditOferta.js')}}"> </script>
@endsection